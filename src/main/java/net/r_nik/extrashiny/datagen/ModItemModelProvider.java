...
public class ModItemModelProvider extends ItemModelProvider {
    ...
    private void trimmedArmorItem(DeferredHolder<Item, Item> itemRegistryObject) {
        final String MOD_ID = ExtraShiny.MOD_ID;
    ...
    private void entityItem(DeferredHolder<Item, Item> item) {
    ...
    private ItemModelBuilder simpleItem(DeferredHolder<Item, Item> item) {
        return withExistingParent(item.getId().getPath(), ResourceLocation.parse("item/generated")).texture("layer0", ResourceLocation.fromNamespaceAndPath(ExtraShiny.MOD_ID, "item/" + item.getId().getPath()));
    }

    private ItemModelBuilder horseArmorItem(DeferredHolder<Item, Item> item) {
        return withExistingParent(item.getId().getPath(), ResourceLocation.parse("item/generated")).texture("layer0", ResourceLocation.fromNamespaceAndPath(ExtraShiny.MOD_ID, "item/" + item.getId().getPath()));
    }


    private ItemModelBuilder handheldItem(DeferredHolder<Item, Item> item) {
        return withExistingParent(item.getId().getPath(), ResourceLocation.parse("item/handheld")).texture("layer0", ResourceLocation.fromNamespaceAndPath(ExtraShiny.MOD_ID, "item/" + item.getId().getPath()));
    }

    private void blockItem(DeferredHolder<net.minecraft.world.level.block.Block, ? extends net.minecraft.world.level.block.Block> block) {
        withExistingParent(
                block.getId().getPath(),
    ...
    private void wallBlockItem(
            DeferredHolder<net.minecraft.world.level.block.Block, ? extends net.minecraft.world.level.block.Block> wall,
            DeferredHolder<net.minecraft.world.level.block.Block, ? extends net.minecraft.world.level.block.Block> baseBlock
    ) {
        withExistingParent(
                wall.getId().getPath(),
